...
// TestAsyncTerraformPluginFrameworkConnectDetachesSetupContext ensures that
// the context passed to the configured terraform.SetupFn is not canceled
...
// lifetime of the reconciliation request that called Connect.
func TestAsyncTerraformPluginFrameworkConnectDetachesSetupContext(t *testing.T) {
	...
	c := NewTerraformPluginFrameworkAsyncConnector(nil, ots, setupFn, newBaseUpjetConfig(), WithTerraformPluginFrameworkAsyncLogger(logTest))
